...
class Jenis_pengeluaran extends CI_Controller {
	...
	public function tambah()
	{
		...
		$data['master'] = 'active';
			...
			$this->load->view('script/modal-jenis_pengeluaran');
			$this->load->view('script/notif');
			$this->load->view('validasi/jenis_pengeluaran/form_tambah_jenis_pengeluaran');
	...
	public function ubah($kd_jenis_pengeluaran)
	{
		...
		$data['jenis_pengeluaran_item'] = $this->jenis_pengeluaran_model->get_jenis_pengeluaran($kd_jenis_pengeluaran);
			...
			$this->load->view('script/modal-jenis_pengeluaran');
			$this->load->view('script/notif');
			$this->load->view('validasi/jenis_pengeluaran/form_ubah_jenis_pengeluaran');
	...
	public function cek_kode()
	{
		...
		$kd_jenis_pengeluaran = $this->jenis_pengeluaran_model->get_kd_jenis_pengeluaran($kode);

		if (count($kd_jenis_pengeluaran) == 1) {
			$respone = TRUE;
		}
		else {
			$respone = FALSE;
	...
	public function cek_kode2()
	{
		...
		if (count($kd_jenis_pengeluaran) == 1) {
			if ($kd_jenis_pengeluaran['kd_jenis_pengeluaran'] != $cek) {
				$respone = TRUE;
			}
			else {
				$respone = FALSE;
