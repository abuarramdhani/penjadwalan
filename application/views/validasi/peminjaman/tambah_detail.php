...
<script type="text/javascript">
  ...
  $('#simpan').click(function(){
    $.ajax({
      url: "<?php echo site_url('transaksi_vendor/tombol_simpan') ?>",
      dataType: "JSON",
      cache: false,
      beforeSend:function(){
      ...
      success: function(result){
        if (result.status) {
          ...
          window.location.href = "<?php echo site_url('peminjaman/peminjaman?notif=Data peminjaman berhasil disimpan&alert=success&logo_notif=check') ?>";
        }
        else {
          $('#modal-loading').modal('hide');
          $('.alert').show();
