    ...
    $.ajax({
      ...
      success: function(result){
        if (result) {
          ...
          $.ajax({
            type: 'POST',
            url: '<?php echo site_url('Peralatan/tombol_ubah/'.$peralatan_item['kd_peralatan']) ?>',
            data: {kode1: kode1, kode: kode, tipe: tipe, status: status},
            cache: false,
            success: function(result){
              if (kode1 != '') {
                window.location.href = "<?php echo site_url('peralatan/peralatan?notif=Data peralatan berhasil diubah&alert=success&logo_notif=check') ?>";
              }
            }
          })
        }
