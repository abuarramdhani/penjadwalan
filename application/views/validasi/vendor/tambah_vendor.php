  ...
  if(email == "")
    {
      ...
      $.ajax({
        type: 'POST',
        url: '<?php echo site_url('Vendor/tombol_simpan') ?>',
        data: {nama: nama, hp: hp, alamat: alamat, email: email, status: status},
        cache: false,
        success: function(result){
          if (nama != '' && hp != '' && alamat != '') {
            window.location.href = "<?php echo site_url('vendor/vendor?notif=Data vendor berhasil disimpan&alert=success&logo_notif=check') ?>";
          }
        }
      })
    }
