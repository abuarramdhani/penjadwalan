...
<script type="text/javascript">
  ...
  $('#ubah').click(function(){
    var back = '<?php echo $_GET['back']; ?>';
    $.ajax({
      url: '<?php echo site_url('transaksi_klien/tombol_ubah_detail/'.$_SESSION['kd_pemesanan']) ?>',
      dataType: 'JSON',
      cache: false,
      beforeSend: function() {
      ...
      success: function(result){
        if (result.status) {
          if (back == 'rab') {
            ...
            window.location.href = "<?php echo site_url('rab/'.$_SESSION['kd_pemesanan'].'?notif=Data pemesanan berhasil diubah&alert=success&logo_notif=check') ?>";
          }
          else {
          $('#modal-loading').modal('hide');
