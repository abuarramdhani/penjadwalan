<script type="text/javascript">
  ...
  $("#simpan").click(function(){
    ...
    $.ajax({
      type: 'POST',
      url: '<?php echo site_url('Area/tombol_simpan') ?>',
      data: {nama: nama, uang_makan: uang_makan, status: status},
      cache: false,
      success: function(result){
        if (nama != '' && uang_makan != '' && status != '') {
          window.location.href = "<?php echo site_url('area/area?notif=Data area berhasil disimpan&alert=success&logo_notif=check') ?>";
        }
      }
    })
  });
