...
$(document).ready(function(){
  ...
  $('#email').keyup(function(){
    var email = $('#email').val();
    var cek = '<?php echo $klien_item['email'] ?>';

    $.ajax({
      type: 'POST',
      url: '<?php echo site_url('Klien/cek_email2') ?>',
      dataType: 'JSON',
  ...
  $('#company').keyup(function() {
    var $th = $(this);
    $th.val( $th.val().replace(/[^a-zA-Z ]/g, function(str) {
          $(".form-group.company.col-md-12").attr("class", "form-group company has-error col-md-12");
          $("label#company_error").hide();
          $("label#company_vali").show();
          return "";
    } ) );
  });

  $('#company').keydown(function() {
        $(".form-group.company.col-md-12.has-error").attr("class", "form-group company col-md-12");
        $('.error').hide();
        $('.vali').hide();
        return true;
        return false;
