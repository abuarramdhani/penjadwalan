<script type="text/javascript">
  ...
  $("#ubah").click(function(){
    ...
    if(email == "")
    {
      ...
      $.ajax({
        type: 'POST',
        url: '<?php echo site_url('Klien/tombol_ubah/'.$klien_item['kd_klien']) ?>',
        data: {nama: nama, hp: hp, alamat: alamat, email: email, company: company, status: status},
        cache: false,
        success: function(result){
          if (nama != '' && hp != '' && alamat != '') {
            window.location.href = "<?php echo site_url('klien/klien?notif=Data klien berhasil diubah&alert=success&logo_notif=check') ?>";
          }
        }
      })
    }
