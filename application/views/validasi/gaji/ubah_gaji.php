<script type="text/javascript">
  ...
  $("#ubah").click(function(){
    ...
    $.ajax({
      type: 'POST',
      url: '<?php echo site_url('Gaji/tombol_ubah/'.$gaji_item['kd_gaji']) ?>',
      data: {gaji: gaji, magang: magang, pekerjaan: pekerjaan, area: area, status: status},
      cache: false,
      success: function(result){
        if (gaji != '' && magang != '') {
          window.location.href = "<?php echo site_url('gaji/gaji?notif=Data gaji berhasil diubah&alert=success&logo_notif=check') ?>";
        }
      }
    })
  });
