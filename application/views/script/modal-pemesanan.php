<!--Modal Pemesanan-->
<script>
$('#modal-pemesanan').on('show.bs.modal', function (event) {
...
modal.find('#pekerjaan_saya').attr('href', '<?php echo site_url(); ?>pekerjaan_saya/'+ a)

if (b == 'Alat & Jasa') {
  modal.find('#rab').attr('href', '<?php echo site_url(); ?>rab/'+ a)
  modal.find('#rab').text('RAB')
}
else {
  modal.find('#rab').attr('href', '<?php echo site_url(); ?>rab2/'+ a)
  modal.find('#rab').text('Laporan')
}
})
</script>
